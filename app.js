...
async function fetchWeather() {
  ...
  try {
    const [curr, fcst] = await Promise.all([
      apiFetch(base + '&current=temperature_2m,weather_code,wind_speed_10m,apparent_temperature'),
      apiFetch(base + '&daily=temperature_2m_max,temperature_2m_min,weather_code,precipitation_probability_max&start_date=2026-05-30&end_date=2026-06-06'),
    ]);
...
// ── Live Rates (Frankfurter/ECB, free, no key) ────────────────────────────────
...
async function fetchRates() {
  try {
    ...
    const data = await apiFetch(`https://api.frankfurter.app/latest?base=EUR&symbols=${syms}`);
